...
eval.results<-confusionMatrix(pred.tree, test.data$diabetes, positive = "Yes")
print(eval.results)
...
analysis1 <- roc(response=test.data$diabetes, predictor=pred.tree.prob[,2])
...
#Another potential evaluation: Area under the Receiver Operating Curve (AUROC)
analysis2 <- roc(response=test.data$diabetes, predictor=pred.svm.prob[,2])

```

## Logistic Regression

```{r model_evaluation_log, message=FALSE, warning=FALSE}

#Create predictions in test set
pred.log <- logistic |>
              predict(test.data)

eval.results<-confusionMatrix(pred.log, test.data$diabetes, positive = "Yes")
print(eval.results)

#Create predictions as probabilities on test set 
pred.log.prob <- logistic %>% 
  predict(test.data, type = "prob")

#Another potential evaluation: Area under the Receiver Operating Curve (AUROC)
analysis3 <- roc(response=test.data$diabetes, predictor=pred.log.prob[,2])
```

## ROC curve of the three models

```{r ROC_curve, warning=FALSE, message=FALSE}
plot(1-analysis1$specificities,analysis1$sensitivities,type="l",
ylab="Sensitivity",xlab="1-Specificity",col="black",lwd=2,
main = "ROC Curve for three models")
lines(1-analysis2$specificities,analysis2$sensitivities,type="l",col="red",lwd=2)
lines(1-analysis3$specificities,analysis3$sensitivities,type="l",col="blue",lwd=2)
abline(a=0,b=1)
```
